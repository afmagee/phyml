...
void Connect_One_Edge_To_Two_Nodes(t_node *a, t_node *d, t_edge *b, t_tree *tree)
{
  ...
  else if(d->tax) b->num = d->num;
  else if(a->tax) b->num = a->num;
  else assert(FALSE);
  ...
  if(!precise)
    {
      ...
      if(expt == YES) for(i=0;i<n_param;i++) param[i] = log(param[i]);
      ...
      if(expt == YES) for(i=0;i<n_param;i++) param[i] = exp(param[i]);
      ...
      for(i=1;i<n_iter;i++)
        {
          ...
          if(expt == YES) for(j=0;j<n_param;j++) param[j] = log(param[j]);
          ...
          if(expt == YES) for(j=0;j<n_param;j++) param[j] = exp(param[j]);
...
int Num_Derivative_Several_Param(t_tree *tree, phydbl *param, int n_param, phydbl stepsize, short int logt, short int expt,
...
void Record_Br_Len(t_tree *mixt_tree)
{
  ...
  do
    {
      for(i=0;i<2*tree->n_otu-1;++i) tree->a_edges[i]->l_old->v     = tree->a_edges[i]->l->v;
      for(i=0;i<2*tree->n_otu-1;++i) tree->a_edges[i]->l_var_old->v = tree->a_edges[i]->l_var->v;
      tree = tree->next;
    }
  while(tree);
}
...
void Restore_Br_Len(t_tree *mixt_tree)
{
  ...
  do
    {
      for(i=0;i<2*tree->n_otu-1;++i) tree->a_edges[i]->l->v     = tree->a_edges[i]->l_old->v;
      for(i=0;i<2*tree->n_otu-1;++i) tree->a_edges[i]->l_var->v = tree->a_edges[i]->l_var_old->v;
      tree = tree->next;
    }
  while(tree);
}
...
void Get_Node_Ranks_From_Tip_Times(t_tree *tree)
{
  ...
  for(i=0;i<tree->n_otu-1;++i) tree->a_nodes[rk[i]]->rk_next = tree->a_nodes[rk[i+1]];
  for(i=0;i<tree->n_otu-1;++i) tree->a_nodes[rk[i+1]]->rk_prev = tree->a_nodes[rk[i]];
...
{
  int i;
  for(i=0;i<2*tree->n_otu-3;++i) tree->a_edges[i]->l->v = log(tree->a_edges[i]->l->v);
}
          ...
          do
            {
              ...
              Generic_Brent_Lk(&(tree->norm_scale),
                               ...
                               10000,
                               NO,
                               Wrap_Diff_Lk_Norm_At_Given_Edge,tree->a_edges[i],tree,NULL,NO,NO);
      ...
      if(b != tree->e_root)
          ...
          if(ta < td)
            {
              ...
              *target_time = ta + delta / (tree->rates->cur_l[d->num] / fabs(ta-td));
              *target_nd = d;
              /* printf("\n$ %G %G", */
          ...
          else
            {
              ...
              *target_time = ta - delta / (tree->rates->cur_l[a->num] / fabs(ta-td));
              *target_nd = a;
              /* printf("\nz %G %G", */
      ...
      else
        {
          ...
          if(delta < tree->rates->cur_l[a->num])
            {
              /* *target_time = ta - delta / (tree->rates->clock_r * tree->rates->br_r[a->num]); */
              *target_time = ta - delta / (tree->rates->cur_l[a->num] / fabs(t_root-ta));
              *target_nd = a;
